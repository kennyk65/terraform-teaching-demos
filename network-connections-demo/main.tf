...
#  Find the peering resource in the management console and accept the connection. 
...
resource "aws_ec2_transit_gateway" "tgw" {
  count                          = local.create_transit_gateway # Only create this resource if we want to build a transit gateway demo
  auto_accept_shared_attachments = "enable"
}

resource "aws_ec2_transit_gateway_vpc_attachment" "consumer_attachment" {
  count              = local.create_transit_gateway # Only create this resource if we want to build a transit gateway demo
...
resource "aws_ec2_transit_gateway_vpc_attachment" "provider_attachment" {
  count              = local.create_transit_gateway # Only create this resource if we want to build a transit gateway demo
  subnet_ids         = [aws_subnet.ProviderSubnet.id]
  transit_gateway_id = aws_ec2_transit_gateway.tgw[0].id
  vpc_id             = aws_vpc.ProviderVPC.id
}

resource "aws_route" "consumer_transit_route" {
  count                  = local.create_transit_gateway # Only create this resource if we want to build a transit gateway demo
  depends_on             = [aws_ec2_transit_gateway_vpc_attachment.consumer_attachment]
  transit_gateway_id     = aws_ec2_transit_gateway.tgw[0].id
  route_table_id         = aws_route_table.ConsumerRouteTable.id
  destination_cidr_block = "10.0.0.0/16"
}

resource "aws_route" "provider_transit_route" {
  count                  = local.create_transit_gateway # Only create this resource if we want to build a transit gateway demo
  depends_on             = [aws_ec2_transit_gateway_vpc_attachment.provider_attachment]
  transit_gateway_id     = aws_ec2_transit_gateway.tgw[0].id
  route_table_id         = aws_route_table.ProviderRouteTable.id
  destination_cidr_block = "172.16.0.0/16"
}
